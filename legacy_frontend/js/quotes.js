...
const QuotesModule = {
  ...
  calcQuote() {
    ...
    const systemCost    = Math.round(kw * PRICING.costPerKW);
    ...
    const subsidy       = applySubsidy ? Math.min(Math.round(kw * PRICING.subsidyPerKW), 78000) : 0;
    const netCost       = totalCost - subsidy;
    const annualGen     = Math.round(kw * PRICING.annualGenPerKW);
    const annualSaving  = Math.round(annualGen * PRICING.electricityCostPerUnit);
    const payback       = (netCost / annualSaving).toFixed(1);
    const co2Saving     = Math.round(annualGen * 0.82); // kg CO2/kWh

    return { kw, systemCost, installCost, gst, totalCost, subsidy, netCost, annualGen, annualSaving, payback, co2Saving };
  },
        ...
        <div class="quote-body">
          ...
          <table class="quote-table">
            ...
            <tbody>
              ...
              <tr class="quote-total-row">
                <td colspan="4" style="font-size:15px;">NET AMOUNT PAYABLE</td>
      ...
      <div style="margin-bottom:16px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          ${[
            ...
            ['PM Subsidy', '-' + Utils.formatINR(lead.subsidy), 'var(--green-400)'],
          ].map(([l,v,c]) => `
        ...
        <div style="background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.3);border-radius:var(--radius-lg);padding:16px;text-align:center;margin-top:12px;">
          <div style="font-size:12px;color:var(--text-muted);">NET AMOUNT PAYABLE</div>
          <div style="font-size:28px;font-weight:800;color:var(--gold-400);">${Utils.formatINR(lead.netCost)}</div>
          <div style="font-size:12px;color:var(--text-muted);margin-top:4px;">
            Annual Saving: <strong style="color:var(--green-400);">₹${Math.round(lead.kwSize * PRICING.annualGenPerKW * PRICING.electricityCostPerUnit).toLocaleString('en-IN')}</strong> | 
            Payback: <strong style="color:var(--teal-400);">${(lead.netCost / (lead.kwSize * PRICING.annualGenPerKW * PRICING.electricityCostPerUnit)).toFixed(1)} yrs</strong>
          </div>
        </div>
      </div>
    `;
